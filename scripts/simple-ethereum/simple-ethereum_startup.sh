...
nohup geth --dev --datadir "$DATADIR" -mine --miner.gaslimit "$GAS_PRICE" --miner.gasprice "$GAS_LIMIT" --http --http.addr "$HTTP_ADDR" --http.port "$HTTP_PORT" --http.api "$HTTP_API" > "$DATADIR"/log 2>&1 &
